"""
lstm_LOSO_cross-val_sliding_window.py - WINDOWED VERSION

LOSO Cross-Validation for windowed sequences.
Leave-One-Sequence-Out: Hold out all windows from one (original_sequence, camera) combination.

Key differences from non-windowed:
- Works with fixed-length windows instead of variable-length sequences
- Uses timeline reconstruction for evaluation
- Applies same causal post-processing as LOCO windowed version
"""
import os
...
OUT_DIR = "checkpoints_loso_windowed"
...
print(f"\n{'='*80}")
print(f"WINDOWED LOSO CROSS-VALIDATION")
print(f"{'='*80}")
print(f"Dataset: {len(dataset)} windows")
print(f"Window size: {dataset.window_size} frames")
print(f"Strategy: LOSO (Leave-One-Sequence-Out) - holds out all windows from one (orig_seq, cam)\n")
...
# Group CV windows by (original_sequence, camera)
seqcam_to_windows = {}
for idx in cv_indices:
    orig_seq, cam, _, _ = dataset.meta[idx]
    key = (orig_seq, cam)
    if key not in seqcam_to_windows:
        seqcam_to_windows[key] = []
    seqcam_to_windows[key].append(idx)

print(f"Found {len(seqcam_to_windows)} unique (sequence, camera) combinations in CV set")
...
# LOSO: Leave out all windows from one (orig_seq, cam) at a time
for fold_idx, test_seqcam in enumerate(sorted(seqcam_to_windows.keys())):
    test_seq, test_cam = test_seqcam
    
    # Validation: all windows from this (orig_seq, cam)
    val_idx = seqcam_to_windows[test_seqcam]
    
    # Training: all windows from other (orig_seq, cam) combinations
    train_idx = []
    for seqcam, windows in seqcam_to_windows.items():
        if seqcam != test_seqcam:
            train_idx.extend(windows)
    
    cam_short = str(test_cam)[-4:] if len(str(test_cam)) > 4 else str(test_cam)
    fold_name = f"seq_{fold_idx:02d}"

    print(f"\n=== LOSO Fold {fold_idx+1}/{len(seqcam_to_windows)}: test ({test_seq}, cam {cam_short}) ===")
    print(f"Train: {len(train_idx)} windows | Val: {len(val_idx)} windows")
    
    res = train_and_eval_fold(dataset, train_idx, val_idx, fold_name)
    results[test_seqcam] = res
...
if held_out_indices:
    ...
    best_seqcam = max(results.keys(), key=lambda s: results[s]['frame_f1'])
    best_fold_idx = sorted(seqcam_to_windows.keys()).index(best_seqcam)
    best_model_path = f"{OUT_DIR}/best_seq_{best_fold_idx:02d}.pt"
    
    print(f"\nUsing best model from LOSO fold {best_fold_idx} (Frame F1={results[best_seqcam]['frame_f1']:.3f})")
...
print("="*60)
print(f"\n{'Seq':<10} {'Camera':<10} {'Frame P':<9} {'Frame R':<9} {'Frame F1':<9} {'Event P':<9} {'Event R':<9} {'Event F1':<9} {'Thr':<5}")
print("-" * 95)

for seqcam in sorted(results.keys()):
    r = results[seqcam]
    seq, cam = seqcam
    cam_short = str(cam)[-4:] if len(str(cam)) > 4 else str(cam)
    print(f"{seq:<10} {cam_short:<10} {r['frame_p']:<9.3f} {r['frame_r']:<9.3f} {r['frame_f1']:<9.3f} "
...
ax.set_title('LOSO (Windowed): F1 per Fold', fontsize=13, fontweight='bold')
...
plt.tight_layout()
plt.savefig('loso_cv_results_lstm_windowed.png', dpi=150, bbox_inches='tight')
print(f"\n📊 Plot saved: loso_cv_results_lstm_windowed.png")

loso_results = {
    'approach': 'LOSO_windowed',
    'n_folds': len(results),
...
with open('loso_cv_results_lstm_windowed.pkl', 'wb') as f:
    pickle.dump(loso_results, f)
print(f"💾 Results saved: loso_cv_results_lstm_windowed.pkl")

print("\n✅ Windowed LOSO Complete!")
print(f"\nKey differences from non-windowed:")
print(f"  - {len(results)} folds (one per (sequence, camera) combination)")
